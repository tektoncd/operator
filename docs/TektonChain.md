...
- `generateSigningSecret`: If set to true, the operator will generate a cosign key pair (`cosign.key` private key and `cosign.pub` public key) and store them in the `signing-secrets` secret in the `tekton-pipelines` namespace. This secret is used by the Chains controller to sign Tekton artifacts (taskruns, pipelineruns). It is important to note that:
 * The user should retrieve and store the `cosign.pub` public key in a secure location to verify artifact attestations.
