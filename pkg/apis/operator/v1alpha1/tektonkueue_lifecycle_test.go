...
func TestTektonKueueStatus_SuccessConditions(t *testing.T) {
	tt := &TektonKueueStatus{}
...
func TestTektonKueueStatus_ErrorConditions(t *testing.T) {
	// Given
	tps := &TektonKueueStatus{}

	tps.MarkPreReconcilerFailed("Reconciliation Failed for Kueue")
	apistest.CheckConditionFailed(tps, PreReconciler, t)

	// Not Ready Condition
	tps.MarkNotReady("Kueue Not Ready")
	apistest.CheckConditionFailed(tps, apis.ConditionReady, t)

	// PostReconciler Failed Condition
	tps.MarkPostReconcilerFailed("Kueue PostReconciler Failed")
	apistest.CheckConditionFailed(tps, PostReconciler, t)

	// InstallerSetNotAvailable Condition
	tps.MarkInstallerSetNotAvailable("Kueue InstallerSetNotAvailable ")
	apistest.CheckConditionFailed(tps, InstallerSetAvailable, t)

	// InstallerSetNotAvailable Condition
	tps.MarkInstallerSetNotReady("Kueue InstallerSetNotReady ")
	apistest.CheckConditionFailed(tps, InstallerSetReady, t)

	// DependencyInstalling Condition
	tps.MarkDependencyInstalling("Kueue Dependencies are installing ")
	apistest.CheckConditionFailed(tps, DependenciesInstalled, t)

	// DependencyMissing Condition
	tps.MarkDependencyMissing("Kueue Dependencies are Missing ")
	apistest.CheckConditionFailed(tps, DependenciesInstalled, t)
}
