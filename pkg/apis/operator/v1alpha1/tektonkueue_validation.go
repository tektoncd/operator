...
func (ts *TektonKueue) Validate(ctx context.Context) (errs *apis.FieldError) {

	if apis.IsInDelete(ctx) {
		return nil
	}

	if ts.GetName() != TektonKueueResourceName {
		errMsg := fmt.Sprintf("metadata.name,  Only one instance of TektonKueue is allowed by name, %s", TektonKueueResourceName)
