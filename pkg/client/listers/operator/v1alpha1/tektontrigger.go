...
type TektonTriggerLister interface {
	...
	List(selector labels.Selector) (ret []*operatorv1alpha1.TektonTrigger, err error)
	// Get retrieves the TektonTrigger from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*operatorv1alpha1.TektonTrigger, error)
...
type tektonTriggerLister struct {
	listers.ResourceIndexer[*operatorv1alpha1.TektonTrigger]
}
...
func NewTektonTriggerLister(indexer cache.Indexer) TektonTriggerLister {
	return &tektonTriggerLister{listers.New[*operatorv1alpha1.TektonTrigger](indexer, operatorv1alpha1.Resource("tektontrigger"))}
}
