...
type TektonInstallerSetLister interface {
	...
	List(selector labels.Selector) (ret []*operatorv1alpha1.TektonInstallerSet, err error)
	// Get retrieves the TektonInstallerSet from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*operatorv1alpha1.TektonInstallerSet, error)
...
type tektonInstallerSetLister struct {
	listers.ResourceIndexer[*operatorv1alpha1.TektonInstallerSet]
}
...
func NewTektonInstallerSetLister(indexer cache.Indexer) TektonInstallerSetLister {
	return &tektonInstallerSetLister{listers.New[*operatorv1alpha1.TektonInstallerSet](indexer, operatorv1alpha1.Resource("tektoninstallerset"))}
}
