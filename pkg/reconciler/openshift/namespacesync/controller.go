...
func NewController(ctx context.Context, _ configmap.Watcher) *controller.Impl {
	...
	// This controller only needs Secret name/namespace/labels to evaluate binding
	// rules. Strip the (potentially large, potentially sensitive) Data/StringData
	// payload before Secrets are stored in the shared informer cache, so the
	// operator process does not hold the contents of every Secret in the cluster
	// in memory just to watch for name/label matches.
	if err := secretInf.Informer().SetTransform(stripSecretData); err != nil {
		logger.Panicf("Couldn't set Secret informer transform: %v", err)
	}
	...
	if _, err := nsInf.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		// Delete → reconcile once more so ensureClusterInterceptorsSubject removes
		// this namespace's subject from the cluster-scoped ClusterInterceptors
		// ClusterRoleBinding. Namespace-scoped resources are garbage collected by
		// Kubernetes automatically and need no explicit cleanup.
		//
	...
	if _, err := secretInf.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		// Update → a label change can cause a Secret to start or stop matching a
		// label-selector binding rule. Re-enqueue if either the old or the new
		// state matches so the corresponding secret is bound or unbound.
		UpdateFunc: func(oldObj, newObj interface{}) {
			newSecret, ok := newObj.(*corev1.Secret)
			if !ok {
				return
			}
			matches := secretMatchesBinding(newSecret, tcInf.Lister())
			if !matches {
				if oldSecret, ok := oldObj.(*corev1.Secret); ok {
					matches = secretMatchesBinding(oldSecret, tcInf.Lister())
				}
			}
			if matches {
				impl.EnqueueKey(types.NamespacedName{Name: newSecret.Namespace})
			}
		},
		DeleteFunc: func(obj interface{}) {
			secret, ok := obj.(*corev1.Secret)
...
// stripSecretData is a cache.TransformFunc that removes the contents of a
// Secret before it is stored in the shared informer cache. Only metadata
// (name, namespace, labels) is needed to evaluate binding rules; keeping full
// Secret payloads resident in the operator process for every Secret in the
// cluster is unnecessary memory overhead and needlessly exposes sensitive
// data to a wider surface.
