...
// TestReconcile_DefaultsAppliedForPreExistingTektonConfig simulates a
// TektonConfig that was persisted before this controller existed: on
// OpenShift, spec.platforms.openshift.namespaceSync is absent from the stored
// object entirely (nil), exactly as it would be for a CR that was never
// spec-edited after the operator upgraded. Reconcile must apply defaulting
// in-memory and keep syncing rather than treating the nil block as "feature
// disabled".
func TestReconcile_DefaultsAppliedForPreExistingTektonConfig(t *testing.T) {
	...
	// CreateEditRoleBinding also defaults to true, so the reconciler needs the
	// edit ClusterRole to exist before it will create the RoleBinding.
	...
	// CreatePipelineSA defaults to true once defaulting is applied, so the
	// pipeline SA must have been created even though the stored TektonConfig
	// never had spec.platforms.openshift.namespaceSync set.
...
// TestReconcile_NamespaceDeleted_RemovesClusterInterceptorsSubject verifies
// that reconciling a namespace which no longer exists in the lister (as
// happens once the informer processes its Delete event) removes its pipeline
// SA subject from the cluster-scoped ClusterInterceptors ClusterRoleBinding
// instead of just returning nil, since that cluster-scoped resource is not
// garbage collected automatically when the namespace goes away.
func TestReconcile_NamespaceDeleted_RemovesClusterInterceptorsSubject(t *testing.T) {
	...
	assert.NilError(t, operatorInfs.Operator().V1alpha1().TektonConfigs().Informer().GetStore().Add(tc))
	nsA := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "ns-a"}}
	nsB := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "ns-b"}}
	assert.NilError(t, nsInformer.Informer().GetStore().Add(nsA))
	assert.NilError(t, nsInformer.Informer().GetStore().Add(nsB))
	...
	// Remove ns-a from the lister's store and delete its pipeline SA — as
	// Kubernetes garbage collection would once the namespace is fully gone —
	// then reconcile it again, as the informer's Delete handler would.
	assert.NilError(t, nsInformer.Informer().GetStore().Delete(nsA))
...
func TestNamespaceMatchesSelector_NilSelectorMatchesAll(t *testing.T) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "my-ns"}}
	cfg := &v1alpha1.NamespaceSyncConfig{}
...
func TestNamespaceMatchesSelector_MatchLabelsExcludesNonMatching(t *testing.T) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "my-ns", Labels: map[string]string{"team": "b"}}}
	cfg := &v1alpha1.NamespaceSyncConfig{
		NamespaceSelector: &metav1.LabelSelector{MatchLabels: map[string]string{"team": "a"}},
	}
...
// A malformed selector must fail closed (match nothing) so that a typo in the
// selector cannot silently widen namespace sync to the entire cluster.
func TestNamespaceMatchesSelector_MalformedSelectorFailsClosed(t *testing.T) {
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "my-ns", Labels: map[string]string{"team": "a"}}}
	cfg := &v1alpha1.NamespaceSyncConfig{
...
func TestEnsureEditRoleBinding_CreatesWhenAbsent(t *testing.T) {
	...
	// New RoleBindings must carry the TektonConfig owner reference so they are
	// garbage collected when the TektonConfig CR is deleted.
	assert.Equal(t, 1, len(rb.OwnerReferences))
	assert.Equal(t, v1alpha1.ConfigResourceName, rb.OwnerReferences[0].Name)
}

func TestEnsureEditRoleBinding_CorrectsDriftOnExisting(t *testing.T) {
	tc := minimalTC(&v1alpha1.NamespaceSyncConfig{
		CreatePipelineSA:      boolPtr(false),
		CreateEditRoleBinding: boolPtr(true),
	})
	...
	// Pre-create the RoleBinding with the correct RoleRef but missing both the
	// owner reference and the pipeline SA subject (as if manually edited).
	_, err := kubeClient.RbacV1().RoleBindings("my-ns").Create(context.Background(), &rbacv1.RoleBinding{
		...
		RoleRef:    rbacv1.RoleRef{APIGroup: rbacv1.GroupName, Kind: "ClusterRole", Name: editClusterRole},
	}, metav1.CreateOptions{})
	assert.NilError(t, err)
...
// TestEnsureSecretBindings_RemovesBindingRemovedFromSpec verifies that a
// secret binding is cleaned up after its rule is removed from
// spec.platforms.openshift.namespaceSync.secretBindings entirely — not just
// when the Secret itself is deleted. This relies on
// managedSecretBindingsAnnotation to recover, on the next reconcile, which
// secrets this controller previously bound.
func TestEnsureSecretBindings_RemovesBindingRemovedFromSpec(t *testing.T) {
	tc := minimalTC(&v1alpha1.NamespaceSyncConfig{
		CreatePipelineSA: boolPtr(false),
		SecretBindings:   []v1alpha1.SecretBinding{{SecretName: "quay-robot"}},
	})
	...
	assert.NilError(t, tcInformer.Informer().GetStore().Add(tc))
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "my-ns"}}
	assert.NilError(t, nsInformer.Informer().GetStore().Add(ns))
	...
	// First reconcile binds quay-robot and records it in the annotation.
	assert.NilError(t, r.Reconcile(context.Background(), "my-ns"))
	sa, err := kubeClient.CoreV1().ServiceAccounts("my-ns").Get(context.Background(), pipelineSA, metav1.GetOptions{})
	assert.NilError(t, err)
	assert.Equal(t, 1, len(sa.ImagePullSecrets))

	// Now clear secretBindings from spec entirely (as if the user removed it)
	// and reconcile again — the binding rule is gone, but the secret must
	// still be removed because it was tracked as managed.
	tc2 := minimalTC(&v1alpha1.NamespaceSyncConfig{CreatePipelineSA: boolPtr(false)})
	assert.NilError(t, tcInformer.Informer().GetStore().Update(tc2))
...
// TestEnsureSecretBindings_LabelBased_RemovesSecretThatStopsMatching verifies
// that a label-based binding is cleaned up symmetrically with named bindings:
// once a secret that used to match the labelSelector stops matching it
// (deleted here; relabeling behaves the same way since ensureSecretBindings
// re-runs the List() query fresh every reconcile), the stale reference is
// removed on the next reconcile even though the labelSelector rule itself
// never changed in spec. This relies on managedSecretBindingsAnnotation to
// remember, across reconciles, which secrets previously matched.
func TestEnsureSecretBindings_LabelBased_RemovesSecretThatStopsMatching(t *testing.T) {
	tc := minimalTC(&v1alpha1.NamespaceSyncConfig{
		CreatePipelineSA: boolPtr(false),
		SecretBindings: []v1alpha1.SecretBinding{{
			LabelSelector: &metav1.LabelSelector{
	...
	// First reconcile: both secrets match, both get bound and recorded.
	assert.NilError(t, r.Reconcile(context.Background(), "my-ns"))
	sa, err := kubeClient.CoreV1().ServiceAccounts("my-ns").Get(context.Background(), pipelineSA, metav1.GetOptions{})
	assert.NilError(t, err)
	assert.Equal(t, 2, len(sa.ImagePullSecrets))

	// secret-b stops matching the selector (deleted here; losing the label
	// would have the identical effect since the List() call is re-evaluated
	// fresh on every reconcile).
	err = kubeClient.CoreV1().Secrets("my-ns").Delete(context.Background(), "secret-b", metav1.DeleteOptions{})
	assert.NilError(t, err)

	// Second reconcile: the labelSelector rule in spec is unchanged, but the
	// stale reference to secret-b must still be removed.
	assert.NilError(t, r.Reconcile(context.Background(), "my-ns"))

	sa, err = kubeClient.CoreV1().ServiceAccounts("my-ns").Get(context.Background(), pipelineSA, metav1.GetOptions{})
	assert.NilError(t, err)
	assert.Equal(t, 1, len(sa.ImagePullSecrets))
	assert.Equal(t, "secret-a", sa.ImagePullSecrets[0].Name)
}
...
// ---------------------------------------------------------------------------
// SCC escalation-prevention (empty maxAllowed security fix)
// ---------------------------------------------------------------------------

// TestSCCAnnotation_EmptyMaxAllowedBlocksEscalation verifies the security fix:
// when maxAllowed is empty, the effective ceiling is the default SCC, so a
...
func TestSCCAnnotation_EmptyMaxAllowedBlocksEscalation(t *testing.T) {
	...
	tests := []struct {
		name            string
		annotationSCC   string // SCC requested by namespace annotation
		defaultSCC      string // TektonConfig.SCC.Default
		maxAllowedSCC   string // TektonConfig.SCC.MaxAllowed ("" = empty)
		wantErr         bool
		wantErrContains string
	}{
		{
			name:            "empty maxAllowed blocks privileged escalation",
			annotationSCC:   "privileged",
			defaultSCC:      "pipelines-scc",
			maxAllowedSCC:   "",
			wantErr:         true,
			wantErrContains: "less restrictive than the effective maxAllowed SCC",
		},
		{
			name:          "empty maxAllowed allows requesting the default SCC itself",
			annotationSCC: "pipelines-scc",
			defaultSCC:    "pipelines-scc",
			maxAllowedSCC: "",
			wantErr:       false,
		},
		{
			name:          "empty maxAllowed allows a MORE restrictive SCC",
			annotationSCC: "restricted",
			defaultSCC:    "pipelines-scc",
			maxAllowedSCC: "",
			wantErr:       false,
		},
		{
			name:          "explicit maxAllowed=privileged allows anyuid",
			annotationSCC: "anyuid",
			defaultSCC:    "pipelines-scc",
			maxAllowedSCC: "privileged",
			wantErr:       false,
		},
		{
			name:            "explicit maxAllowed still blocks over-limit SCC",
			annotationSCC:   "privileged",
			defaultSCC:      "pipelines-scc",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			ns := corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					...
					Annotations: map[string]string{
						"operator.tekton.dev/scc": tt.annotationSCC,
					},
				},
			}
			...
			for i := range sccs {
				_, err := secClient.SecurityV1().SecurityContextConstraints().Create(
					context.Background(), &sccs[i], metav1.CreateOptions{})
				assert.NilError(t, err)
			}

			r, kubeClient := newTestReconciler(t, tc, []corev1.Namespace{ns})
			r.securityClientSet = secClient

			// Create the pipeline SA so SCC role-binding code is reached.
			_, err := kubeClient.CoreV1().ServiceAccounts("test-ns").Create(
				context.Background(),
				&corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: pipelineSA, Namespace: "test-ns"}},
				metav1.CreateOptions{},
			)
			assert.NilError(t, err)

			err = r.Reconcile(context.Background(), "test-ns")
			if tt.wantErr {
