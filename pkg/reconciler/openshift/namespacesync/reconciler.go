...
func (r *Reconciler) ensureSCCRoleBinding(ctx context.Context, ns *corev1.Namespace, tc *v1alpha1.TektonConfig) error {
	...
	if nsSCC == "" {
		...
		// Enforce the maxAllowed SCC policy.
		// Security: treat empty maxAllowed as "only default SCC allowed" to
		// prevent privilege escalation via namespace annotations.  An empty
		// maxAllowed previously allowed ANY SCC to be requested.
		if tc.Spec.Platforms.OpenShift.SCC != nil {
			...
			if effectiveMaxAllowed != "" {
				...
				ok, err := pkgcommon.SCCAMoreRestrictiveThanB(list, nsSCC, effectiveMaxAllowed)
				if err != nil {
					return err
				}
				if !ok {
					return fmt.Errorf("namespace %s requested SCC %s which is less restrictive than the effective maxAllowed SCC %s", ns.Name, nsSCC, effectiveMaxAllowed)
				}
			}
		}
