...
type fakeTektonConfigLister struct {
	tc  *v1alpha1.TektonConfig
	err error
}

func (f *fakeTektonConfigLister) Get(_ string) (*v1alpha1.TektonConfig, error) {
	return f.tc, f.err
}
...
func TestResolveCentralTLSToEnvVars_EnabledButNoLister(t *testing.T) {
	...
	if result != nil {
		t.Errorf("Expected nil when lister uninitialized, got %v", result)
	}
}

func TestGetTLSProfileFromAPIServer_ListerUninitialized(t *testing.T) {
	// Ensure shared lister is nil
	SetSharedAPIServerLister(nil, nil)

	result, err := GetTLSProfileFromAPIServer(context.Background())
	if err != nil {
		t.Errorf("Unexpected error: %v", err)
	}
	if result != nil {
		t.Errorf("Expected nil when lister not initialized, got %v", result)
	}
}
