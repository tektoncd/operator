...
import (
	...
	shipwrightbuildinformer "github.com/tektoncd/operator/pkg/client/injection/informers/operator/v1alpha1/shipwrightbuild"
...
func NewController(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
	return NewExtendedController(NewKubernetesExtension)(ctx, cmw)
}
...
func NewExtendedController(generator common.ExtensionGenerator) injection.ControllerConstructor {
	return func(ctx context.Context, cmw configmap.Watcher) *controller.Impl {
		...
		releaseManifest := manifest.Filter(manifestival.Not(manifestival.ByKind(KindClusterBuildStrategy)))
		strategyManifest := manifest.Filter(manifestival.ByKind(KindClusterBuildStrategy))
		...
		r := &Reconciler{
			...
			installerSetClient:     tektoninstallersetclient.NewInstallerSetClient(tektonInstallerSetInterface, operatorVersion, componentVersion, v1alpha1.KindShipwrightBuild, metrics),
		...
		impl := shipwrightbuild.NewImpl(ctx, r)

		if _, err := shipwrightbuildinformer.Get(ctx).Informer().AddEventHandler(controller.HandleAll(impl.Enqueue)); err != nil {
			logger.Panicf("Couldn't register ShipwrightBuild informer event handler: %w", err)
		}

		if _, err := tektoninstallersetinformer.Get(ctx).Informer().AddEventHandler(cache.FilteringResourceEventHandler{
			FilterFunc: controller.FilterController(&v1alpha1.ShipwrightBuild{}),
