...
func pacSettingsFromTektonPAC(p *v1alpha1.PipelinesAsCode) v1alpha1.PACSettings {
	if p == nil {
		return v1alpha1.PACSettings{}
	}
	return v1alpha1.PACSettings{
		Settings:                 p.Settings,
		AdditionalPACControllers: p.AdditionalPACControllers,
		Options:                  p.Options,
	}
}
...
func updateOPAC(ctx context.Context, opacCR *v1alpha1.OpenShiftPipelinesAsCode, config *v1alpha1.TektonConfig,
	...
	if p := config.Spec.PipelinesAsCodeForCurrentPlatform(); p != nil {
		if !reflect.DeepEqual(opacCR.Spec.PACSettings.Settings, p.PACSettings.Settings) {
			opacCR.Spec.PACSettings.Settings = p.PACSettings.Settings
			updated = true
		}

		if !reflect.DeepEqual(opacCR.Spec.PACSettings.Options, p.PACSettings.Options) {
			opacCR.Spec.PACSettings.Options = p.PACSettings.Options
			updated = true
		}

		if !reflect.DeepEqual(opacCR.Spec.PACSettings.AdditionalPACControllers, p.PACSettings.AdditionalPACControllers) {
