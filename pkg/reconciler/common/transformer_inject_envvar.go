...
const (
	KUBERNETES_MIN_VERSION_ENV_KEY = "KUBERNETES_MIN_VERSION"
)

func DeploymentEnvVarKubernetesMinVersion() mf.Transformer {
	var envVars []corev1.EnvVar
	if minVersion, exists := os.LookupEnv(KUBERNETES_MIN_VERSION_ENV_KEY); exists {
		envVars = append(envVars, corev1.EnvVar{
			Name:  KUBERNETES_MIN_VERSION_ENV_KEY,
			Value: minVersion,
		})
	}
