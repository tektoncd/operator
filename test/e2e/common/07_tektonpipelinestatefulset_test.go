...
// TestTektonPipelinesStatefulset verifies the TektonPipelines creation, statefulset recreation, and TektonPipelines deletion.
func TestTektonPipelinesStatefulset(t *testing.T) {
	...
	if _, err := resources.EnsureTektonPipelineWithStatefulsetExists(clients.TektonPipeline(), crNames); err != nil {
	...
	// Delete the statefulsets one by one to see if they will be recreated.
	t.Run("restore-pipeline-statefulset", func(t *testing.T) {
		resources.AssertTektonPipelineCRReadyStatus(t, clients, crNames)
		resources.DeleteAndVerifyStatefulSet(t, clients, crNames.TargetNamespace, utils.TektonPipelineDeploymentLabel)
		resources.AssertTektonPipelineCRReadyStatus(t, clients, crNames)
	})
