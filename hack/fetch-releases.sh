...
release_yaml() {
	...
	*)
	...
	if [ -f "$dest" ] && [ $FORCE_FETCH_RELEASE = "false" ]; then
		...
		if grep -Eq "$label" $dest || grep -Eq "$label2" $dest || grep -Eq "$label3" $dest; then
...
release_yaml_pac() {
	...
	# Determine file to download based on target
	if [[ ${TARGET} == "kubernetes" ]]; then
		fileSuffix="release.k8s"
	else
		fileSuffix="release"
	fi

	if [[ ${version} == "stable" || ${version} == "nightly" ]]; then
		url="https://raw.githubusercontent.com/openshift-pipelines/pipelines-as-code/${version}/${fileSuffix}.yaml"
	else
		url="https://raw.githubusercontent.com/openshift-pipelines/pipelines-as-code/release-${version}/${fileSuffix}.yaml"
	fi
...
release_yaml_manualapprovalgate() {
	...
	if [ -f "$destinationFile" ] && [ $FORCE_FETCH_RELEASE = "false" ]; then
		if grep -Eq "$version" $destinationFile; then
			echo "release file already exist with required version, skipping!"
			echo ""
		fi
	fi
